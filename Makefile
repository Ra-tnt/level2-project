...
jenkins: jenkins-clone jenkins-up jenkins-test jenkins-admin jenkins-tidy

jenkins-admin:
  kubectl create -n jenkins clusterrolebinding jenkins-account --clusterrole=cluster-admin --serviceaccount=jenkins:jenkins

jenkins-clone:
	git clone https://github.com/M404h/k8s-jenkins
...
elf-clone:
	git clone https://github.com/M404h/elf
...
pro-graf-clone:
	git clone https://github.com/M404h/pro-graf
